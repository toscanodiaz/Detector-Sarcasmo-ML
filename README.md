...
En este proyecto no se aplicó escalamiento numérico tradicional ya que se está trabajando con texto, por lo que es necesario convertirlo en representación numérica para poder aplicar algoritmos de clasificación, entonces se aplicó la capa `TextVectorization` de Keras configurada con `max_tokens = 8000` y `output_sequence_length = 25`. La capa aprende un vocabulario a partir del conjunto de entrenamiento y transforma cada titular en una secuencia de enteros de longitud fija. Una vez adaptada la capa con los datos de entrenamiento se transformaron los conjuntos de train, validation y test, obteniendo matrices con dimensiones (20033, 25), (2862, 25) y (5724, 25) respectivamente. 
...
El artículo también sirvió como base para la comparación final realizada en la novena iteración, donde se implementó una arquitectura alternativa más cercana al enfoque convolucional del trabajo de referencia (embedding + CNN/MaxPooling + capas densas + sigmoid, implementada en el archivo [`sarcasmCNN.py`](https://github.com/toscanodiaz/Detector-Sarcasmo-ML/blob/main/sarcasmCNN.py), lo que permitió evaluar si la extracción de patrones locales mediante convoluciones podía superar al modelo con BiLSTM desarrollado durante el proyecto. Sin embargo los resultados obtenidos mostraron que la octava iteración (basada en BiLSTM) tuvo mejor desempeño en test que la arquitectura CNN/MaxPooling, por lo que se conservó como modelo final.
...
Se implementó una interfaz en el archivo [`UI.py`](https://github.com/toscanodiaz/Detector-Sarcasmo-ML/blob/main/UI.py) utilizando **Gradio**. Esta interfaz carga el modelo entrenado `modelo_dl.keras` y el archivo `preprocessing_metadata.json` donde se recupera el vocabulario, la longitud de secuencia y el mejor threshold seleccionado durante la evaluación.
...
3. Roy, M. C., Bisoy, S. K., Sahoo, P. K., et al. *Advancing news headline sarcasm detection through hybrid neural networks*. Discover Computing, 29, Article 12 (2026).
